...
const Sidebar = () => {
    ...
    const navItems = [
        ...
        { icon: Briefcase,       label: 'Projects',      path: '/projects',      pageId: 'projects' },
    ...
    const filteredNavItems = navItems.filter(item => {
        if (item.pageId === 'dashboard') return true;
        return hasPermission(item.pageId, 1);
    });
                        ...
                        <div className="space-y-0.5 px-3">
                            {projects.slice(0, 5).map(proj => (
                                <NavLink
                                    ...
                                    to={`/projects/${proj.id}`}
                                    className={({ isActive }) =>
                                        `flex items-center px-3 py-2 text-xs rounded-md transition-colors ${isActive ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gh-hover'}`
                                    }
                                >
                                    <Box size={13} className="mr-3 text-blue-500/80" />
                                    <span className="truncate">{proj.name}</span>
                                </NavLink>
                            ))}
                            {projects.length === 0 && (
                                <p className="text-[10px] text-gray-400 px-3 py-1">No assigned projects</p>
                            )}
                        </div>
                    </div>
                )}
