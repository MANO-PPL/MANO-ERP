...
import { FileText, FileSpreadsheet, FileImage, FileStack, ChevronRight, Briefcase, UserCheck, RefreshCw, FileCheck } from 'lucide-react';

const ContractsIndex = ({ setExtraBreadcrumbs }) => {
    const [currentView, setCurrentView] = useState('grid');
    useEffect(() => {
        if (currentView === 'grid') setExtraBreadcrumbs([{ label: 'Contracts' }]);
    }, [currentView, setExtraBreadcrumbs]);

    const categories = [
        { name: 'Prime Contract', icon: <Briefcase size={24} /> },
        { name: 'Sub-contracts', icon: <UserCheck size={24} /> },
        { name: 'Change Orders', icon: <RefreshCw size={24} /> },
        { name: 'Work Orders', icon: <FileCheck size={24} /> },
    ];
                ...
                <div className="w-full space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                        {categories.map((cat, i) => (
                            <div key={i} className="group relative bg-white dark:bg-[#161b22] px-8 py-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 hover:border-blue-500/40 transition-all duration-500 cursor-pointer flex items-center overflow-hidden">
                                ...
                                <div className="relative flex items-center w-full">
                                    ...
                                    <div className="ml-8 flex-1 text-left">
                                        <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 tracking-tight group-hover:text-blue-500 uppercase font-black">{cat.name}</h3>
