...
const PlanningIndex = ({ setExtraBreadcrumbs }) => {
    ...
    const categories = [
        { name: 'Baseline Schedule', icon: <Calendar size={24} />, color: 'blue' },
        { name: 'Master Planning', icon: <BarChart3 size={24} />, color: 'purple' },
        { name: 'Resource Planning', icon: <ClipboardList size={24} />, color: 'orange' },
        { name: 'Material Procurement Plan', icon: <Package size={24} />, color: 'amber' },
    ];
                ...
                <div className="w-full space-y-6">
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                        {categories.map((cat, i) => (
                            <div
                                key={i}
                                className="group relative bg-white dark:bg-gray-50 dark:bg-[#161b22] px-8 py-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 hover:border-blue-500/40 transition-all duration-500 cursor-pointer flex items-center overflow-hidden"
                            >
