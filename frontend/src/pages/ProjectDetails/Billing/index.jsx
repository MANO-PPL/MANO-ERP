...
import { FileText, ChevronRight, Calculator, CreditCard, Receipt, FileSignature } from 'lucide-react';

const BillingIndex = ({ setExtraBreadcrumbs }) => {
    const [currentView, setCurrentView] = useState('grid');
    useEffect(() => {
        if (currentView === 'grid') setExtraBreadcrumbs([{ label: 'Billing' }]);
    }, [currentView, setExtraBreadcrumbs]);

    const categories = [
        { name: 'Client Billing', icon: <Calculator size={24} /> },
        { name: 'Vendor Billing', icon: <CreditCard size={24} /> },
        { name: 'Payment Certificates', icon: <FileSignature size={24} /> },
        { name: 'Tax Invoices', icon: <Receipt size={24} /> },
    ];
