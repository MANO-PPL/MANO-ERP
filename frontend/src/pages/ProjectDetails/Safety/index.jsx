...
import { FileText, ChevronRight, HardHat, MessageSquare, ShieldAlert, AlertTriangle } from 'lucide-react';

const SafetyIndex = ({ setExtraBreadcrumbs }) => {
    const [currentView, setCurrentView] = useState('grid');
    useEffect(() => {
        if (currentView === 'grid') setExtraBreadcrumbs([{ label: 'Safety' }]);
    }, [currentView, setExtraBreadcrumbs]);

    const categories = [
        { name: 'Safety Induction', icon: <HardHat size={24} /> },
        { name: 'Tool Box Talk', icon: <MessageSquare size={24} /> },
        { name: 'Safety Inspections', icon: <ShieldAlert size={24} /> },
        { name: 'Incident Reporting', icon: <AlertTriangle size={24} /> },
    ];
