...
import { FileText, ChevronRight, ShieldCheck, ClipboardCheck, Search, AlertOctagon } from 'lucide-react';

const QualityIndex = ({ setExtraBreadcrumbs }) => {
    const [currentView, setCurrentView] = useState('grid');
    useEffect(() => {
        if (currentView === 'grid') setExtraBreadcrumbs([{ label: 'Quality' }]);
    }, [currentView, setExtraBreadcrumbs]);

    const categories = [
        { name: 'Quality Assurance', icon: <ShieldCheck size={24} /> },
        { name: 'Checklist', icon: <ClipboardCheck size={24} /> },
        { name: 'Inspections', icon: <Search size={24} /> },
        { name: 'Non-Conformance Reports (NCR)', icon: <AlertOctagon size={24} /> },
    ];
