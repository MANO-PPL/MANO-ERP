...
const ClientsList = () => {
    ...
    const saveGridRows = async (customGrid = null) => {
        ...
        try {
            ...
            console.error('Failed to auto-save client grid', err);
            showToast('error', 'Update Error', err.response?.data?.message || 'Failed to save client changes', 5000);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const deleteSelectedRowEntries = async (rowsToDelete) => {
        ...
        setSelectionAnchor(null);
        setSelectionFocus(null);
        showToast('success', 'Entry Deleted', `Deleted ${rowsToDelete.length} client entry(ies).`);

        if (savedIds.length > 0) {
            try {
                await clientApi.deleteClients(savedIds);
            } catch (err) {
                showToast('error', 'Delete Failed', err.response?.data?.message || 'Failed to delete clients', 5000);
            }
        }
    };
    ...
    const handleConfirmDeleteDuplicates = async (idsToDelete) => {
        ...
        if (savedRemovedIds.length > 0) {
            try {
                await clientApi.bulkDeleteClients(savedRemovedIds);
                setClients(prev => prev.filter(c => !savedRemovedIds.includes(c.id)));
            } catch (err) {
                console.error('Error deleting duplicate clients from database', err);
            }
        }

        showToast('success', 'Duplicates Removed', `Successfully deleted ${idsToDelete.length} selected duplicate client(s).`);
    };
    ...
    const handleBulkDelete = () => {
        ...
        setConfirmModal({
            ...
            message: `Are you sure you want to delete ${selectedIds.size} selected client(s)? This action cannot be undone.`,
            ...
            onConfirm: async () => {
                ...
                closeConfirmModal();
                showToast('success', 'Bulk Delete Successful', `Deleted ${count} selected client(s).`);
    ...
    const handleBulkDuplicate = () => {
        ...
        const count = selectedIds.size;
        setSelectedIds(new Set());
        showToast('sparkle', 'Bulk Duplicated', `Created ${count} client duplicate(s).`);
    ...
    const handleBulkChangeSector = (newSectorName) => {
        ...
        closeDropdown();
        showToast('sparkle', 'Sector Updated', `Changed Sector to "${newSectorName}" for ${count} row(s).`);
        triggerAutoSave(updatedGrid, 400);
    };
    ...
    const handleDeleteRow = (row) => {
        ...
        setConfirmModal({
            isOpen: true,
            title: 'Delete Client?',
            message: `Are you sure you want to delete "${row.name || 'this client'}"? This action cannot be undone.`,
            confirmText: 'Delete',
            cancelText: 'Cancel',
            variant: 'danger',
            isLoading: false,
            onConfirm: async () => {
                ...
                closeConfirmModal();
                showToast('success', 'Client Deleted', `Deleted "${row.name || 'Client'}".`);

                try {
                    await clientApi.deleteClient(row.id);
                } catch (err) {
                    showToast('error', 'Delete Failed', err.response?.data?.message || 'Failed to delete client', 5000);
                }
            }
        });
    };
