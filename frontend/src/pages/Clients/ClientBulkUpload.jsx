...
const ClientBulkUpload = () => {
    ...
    const parseCSV = (file) => {
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                try {
                    // Send directly to backend to validate (if needed, or use vendor's endpoint if generic)
                    // For now, mirroring vendor process
                    const res = await api.post('/clients/bulk-validate', { clients: data });
                    ...
                    const processed = data.map((row, idx) => {
                        ...
                        return {
                            ...
                            contact_no: row['Contact No'] || row['contact no'] || row['Telephone'] || row['telephone'] || row['Mobile'] || row['mobile'],
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await api.post('/clients/bulk-json', { clients: validRows });
    ...
    const downloadSample = () => {
        const csvContent = "Name,Contact Person,Designation,Contact No,Email,Location,Address,Website,Category,Job Nature,Sector,Responsibility,Reference,Remarks\n" +
            "Acme Corp,John Smith,CEO,011-223344,john@acme.com,New York,123 Main St,www.acme.com,Corporate,Project Management,Construction,Project Lead,Previous Client,Important Account\n" +
            "Global Tech,Jane Doe,Director,022-556677,jane@globaltech.com,Mumbai,456 Business Park,www.globaltech.com,Individual,IT Services,Technology,Procurement,Referral,N/A";
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 p-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Client Bulk Upload</h2>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                {previewData.slice(0, 50).map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-white/5">
                                        ...
                                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{row.designation || '-'}</td>
