...
const ClientFilter = ({ isOpen, onClose, onApply, currentFilters, availableJobs = [], availableSectors = [] }) => {
    ...
    const filteredSectors = availableSectors.filter(sector =>
        sectorSearch === '' || sector.toLowerCase().includes(sectorSearch.toLowerCase())
    );
    ...
    const handleApply = () => {
        onApply({ jobs: selectedJobs, sectors: selectedSectors });
        onClose();
    };
        ...
        <div className="fixed inset-0 z-[5000] flex justify-end p-0 sm:p-0 anim-fade-in text-left">
            ...
            <div className="relative w-full max-w-xl bg-white dark:bg-[#161b22] shadow-2xl border-l border-gray-200 dark:border-white/10 overflow-hidden flex flex-col h-full anim-slide-left">
                ...
                <div className="p-6 flex flex-col gap-8 custom-scrollbar overflow-y-auto flex-1">
                    {/* Sector Section */}
                    <div>
                        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Sector</h3>
                        ...
                        <div className="h-40 overflow-y-auto custom-scrollbar border border-gray-100 dark:border-white/5 rounded-lg p-2 dark:bg-[#0d1117] space-y-1">
                            {filteredSectors.length > 0 ? filteredSectors.map(sector => (
                                <label key={sector} className="flex items-center space-x-3 p-1.5 hover:bg-gray-100 dark:hover:bg-white/5 rounded cursor-pointer group">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    <span className="text-sm text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{sector}</span>
                                </label>
                            )) : (
                                <p className="text-xs text-center p-4 text-gray-500">No sectors found matching "{sectorSearch}"</p>
                            )}
                        </div>
                    </div>
