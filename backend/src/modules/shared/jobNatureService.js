...
export async function getJobNatures() {
    return await db('crm_job_nature').select('*');
}
...
export async function findOrCreateJobNature(jobName) {
    ...
    const existing = await db('crm_job_nature').where('job_name', trimmed).first();
    if (existing) return existing.job_id;

    const [newId] = await db('crm_job_nature').insert({ job_name: trimmed });
    return newId;
}

export async function createJobNature(jobName) {
    if (!jobName) throw new AppError('Job Nature name is required', 400);
    const existing = await db('crm_job_nature').where({ job_name: jobName }).first();
    if (existing) throw new AppError('Job Nature already exists', 400);
    const [newId] = await db('crm_job_nature').insert({ job_name: jobName });
    return newId;
}
