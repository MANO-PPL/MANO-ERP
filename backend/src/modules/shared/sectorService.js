...
export async function getSectors() {
    return await db('crm_sectors').select('*');
}
...
export async function findOrCreateSector(sectorName) {
    ...
    const existing = await db('crm_sectors').where('sector_name', trimmed).first();
    if (existing) return existing.sector_id;

    const [newId] = await db('crm_sectors').insert({ sector_name: trimmed });
    return newId;
}

export async function createSector(sectorName) {
    if (!sectorName) throw new AppError('Sector name is required', 400);
    const existing = await db('crm_sectors').where({ sector_name: sectorName }).first();
    if (existing) throw new AppError('Sector already exists', 400);
    const [newId] = await db('crm_sectors').insert({ sector_name: sectorName });
    return newId;
}
