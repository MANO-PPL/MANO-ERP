...
const DB_USER = (process.env.DB_USER || 'admin').replace(/^["']|["']$/g, '');
const DB_PASSWORD = (process.env.DB_PASSWORD || '').replace(/^["']|["']$/g, '');
const DB_NAME = (process.env.DB_NAME || 'MANO-ERP').replace(/^["']|["']$/g, '');
...
export async function generateSqlDump() {
    ...
    try {
        ...
        stream.write(`-- ========================================================\n`);
        stream.write(`-- MANO-ERP Database Dump (${includeData ? 'Schema + Data' : 'Schema Only'})\n`);
